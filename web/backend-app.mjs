...
async function useSnapshot(snapshot,offline=false){
  ...
  if(ordinal(snapshot.first)!==range.first||snapshot.days!==range.count){
    ...
    renderer.setMode(snapshot.viewMode||(snapshot.month?'month':'week'),false);
    renderer.select(snapshot.anchor??ordinal(snapshot.first),snapshot.month??true);
  }
  ...
  else if(snapshot.warnings)say('Review source rules in Administration: some entries were not classified or a feed reported a warning. Blank rota days remain unconfirmed.',true);
