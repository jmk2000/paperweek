...
MODULES=['display-controls','config','dates','events','google','renderer','app']
def bundle():
...
def main():
    ...
    with sync_playwright() as p:
        ...
        page.locator('#display-menu > summary').click()
        ...
        check(page.locator('[data-nav="0"] span').inner_text()=='Previous','Touch controls keep visible labels')
        page.locator('[data-nav="3"]').click();page.wait_for_timeout(50)
        page.locator('#display-menu > summary').click()
        page.locator('#settings-button').click()
        check(page.locator('[name="anchor"]').input_value()=='2026-11-01','First touch navigates without hardware help')
        ...
        check(page.locator('[data-build]').first.text_content().startswith('Paperweek '),'Version is available')
        session=page.context.new_cdp_session(page)
        session.send('Emulation.setTouchEmulationEnabled',{'enabled':True,'maxTouchPoints':1})
        before=page.locator('#agenda-text').text_content()
        session.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':600,'y':500}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':400,'y':505}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
        check(page.locator('.swipe-copy').count()==1,'Swipe retains outgoing calendar during animation')
        page.wait_for_timeout(350)
        check(page.locator('.swipe-copy').count()==0,'Swipe cleans up outgoing calendar')
        check(page.locator('#agenda-text').text_content()!=before,'Swipe left navigates to next period')
        after=page.locator('#agenda-text').text_content()
        session.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':400,'y':500}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':420,'y':502}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
        page.wait_for_timeout(350)
        check(page.locator('#agenda-text').text_content()==after,'Short drag snaps back without navigation')
        check(page.locator('.swipe-copy').count()==0,'Cancelled swipe cleans up')
        session.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':400,'y':500}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchMove','touchPoints':[{'x':600,'y':502}]})
        session.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]})
        page.wait_for_timeout(350)
        check(page.locator('#agenda-text').text_content()==before,'Swipe right returns to previous period')
        page.locator('#display-menu > summary').click()
        page.locator('#fullscreen').click();page.wait_for_timeout(100)
        check(page.locator('#fullscreen').text_content()=='Exit full screen','Fullscreen label tracks browser state')
        page.evaluate('document.exitFullscreen()')
        page.wait_for_timeout(100)
        check(page.locator('#fullscreen').text_content()=='Full screen','Exit restores fullscreen label')
        page.evaluate("Object.defineProperty(navigator,'wakeLock',{configurable:true,value:{request:async()=>{globalThis.wakeRequested=true;return {addEventListener(){},release:async()=>{globalThis.wakeReleased=true;}};}}})")
        page.locator('#display-menu > summary').click()
        page.locator('#wake').click()
        check(page.evaluate('globalThis.wakeRequested===true'),'Keep awake requests screen lock')
        page.locator('#wake').click()
        check(page.evaluate('globalThis.wakeReleased===true'),'Keep awake can be turned off')
        if args.screenshots:
            page.locator('#display-menu > summary').click()
            page.screenshot(path=str(args.screenshots/'portrait.png'),full_page=True)
        for width,height in [(412,915),(915,412),(1280,800),(800,1280)]:
            page.set_viewport_size({'width':width,'height':height});page.wait_for_timeout(80)
            check(page.evaluate('document.documentElement.scrollHeight<=innerHeight && document.documentElement.scrollWidth<=innerWidth'),'Viewport fits without page scrolling')
            check(page.locator('.screen-wrap').bounding_box()['height']>=height-76,'Calendar uses nearly the whole viewport')
            check(page.locator('#calendar').evaluate('(c)=>Math.abs(c.width/c.clientWidth-2)<.02 && Math.abs(c.height/c.clientHeight-2)<.02'),'Text uses two drawing pixels per CSS pixel')
            if args.screenshots:page.screenshot(path=str(args.screenshots/f'{width}x{height}.png'))
        page.set_viewport_size({'width':915,'height':412})
        page.locator('#display-menu').evaluate('(el)=>el.open=false')
        page.locator('#display-menu > summary').click()
        page.locator('#fullscreen').click();page.wait_for_timeout(100)
        check(page.evaluate('document.documentElement.scrollHeight<=innerHeight'),'Fullscreen has no vertical scrolling')
        page.evaluate('document.exitFullscreen()');page.wait_for_timeout(100)
        page.locator('.agenda summary').click()
        check(page.locator('.agenda').evaluate('(el)=>el.scrollHeight>el.clientHeight'),'Open agenda can scroll independently')
        page.locator('.agenda summary').click()
        page.locator('#display-menu > summary').click()
        page.locator('#display-menu button',has_text='Install app').click()
        check(page.locator('#install-guide').is_visible(),'Installation fallback is readable in a panel')
        page.locator('.agenda summary').click()
        page.emulate_media(reduced_motion='reduce')
        page.locator('[data-nav="3"]').click();page.wait_for_timeout(80)
        check(page.locator('.swipe-copy').count()==0,'Reduced motion skips slide animation')
        check(not errors,f'No browser errors after navigation and resize: {errors}')
        browser.close()
