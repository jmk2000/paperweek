...
async def run(args):
    ...
    async with async_playwright() as p:
        ...
        await page.evaluate("""()=>{window.slides=[];const animate=Element.prototype.animate;Element.prototype.animate=function(frames,options){window.slides.push({id:this.id,copy:this.classList.contains('swipe-copy'),frames});return animate.call(this,frames,options);};}""")
        box=await page.locator('#planner').bounding_box()
        x,y=box['x']+box['width']/2,box['y']+30
        await page.mouse.move(x,y);await page.mouse.down()
        await page.mouse.move(x-110,y,steps=5)
        await expect(page.locator('.planner-swipe')).to_have_count(1)
        await page.mouse.up()
        await expect(page.locator('.planner-day').first).to_have_attribute('data-date','2026-09-13')
        await expect(page.locator('.swipe-copy')).to_have_count(0)
        assert await page.evaluate("slides.some(s=>s.id==='planner')&&slides.some(s=>s.copy)")
        ...
        passed('Visible planner swipes and navigation slide both views; date taps still open details')
        await load_page()
        ...
        # Prefetch future dates without visiting them, then leave the network.
        await expect(page.locator('#offline-copy')).to_be_checked()
        if not args.in_memory:
            await expect(page.locator('#offline-progress')).to_contain_text('Offline calendars ready:',timeout=30000)
            api.offline=True
            await page.select_option('#view-mode','month')
            for _ in range(3): await page.locator('[data-nav="3"]').click()
            await expect(page.locator('.swipe-copy')).to_have_count(0)
            await expect(page.locator('.planner-heading h1')).to_have_text('December 2026')
            await page.locator('.month-day[data-day="2026-12-25"]').click()
            await expect(page.locator('#day-dialog')).to_contain_text('Working')
            await expect(page.locator('#day-dialog')).to_contain_text('08:00')
            await page.locator('#close-day').click()
            ...
            await expect(page.locator('.swipe-copy')).to_have_count(0)
            await expect(page.locator('.planner-heading h1')).to_have_text('December 2026')
            passed('Offline defaults on; prefetched Christmas rota remains browsable after disconnect and reload')
        else:
            api.offline=True
            await page.locator('[data-nav="3"]').click()
            await expect(page.locator('.swipe-copy')).to_have_count(0)
        ...
        if not args.in_memory:
            await expect(page.locator('#pair-dialog')).to_be_visible()
            remaining=await page.evaluate('''async()=>{const db=await new Promise((resolve,reject)=>{const r=indexedDB.open('paperweek-offline',1);r.onsuccess=()=>resolve(r.result);r.onerror=()=>reject(r.error);});return new Promise(resolve=>{const r=db.transaction('windows').objectStore('windows').count();r.onsuccess=()=>resolve(r.result);});}''')
            assert remaining==0,'Revocation must remove all prefetched dates'
